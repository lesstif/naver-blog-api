...
class HttpClient
{
     ...
     * @throws \Exception
     */
    public function __construct(DotEnvConfiguration $configuration = null, Logger $logger = null, $path = './')
    {
        ...
        if ($logger) {
            ...
            $this->log = new Logger('NaverBlog');
            $this->log->pushHandler(new StreamHandler(
                $configuration->logFile,
                $this->convertLogLevel($configuration->logLevel)
            ));
        }
    ...
    public function exec($context, $post_data = null, $custom_request = null, $cookieFile = null)
    {
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, $this->configuration->curlOptSslVerifyHost);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, $this->configuration->curlOptSslVerifyPeer);
        ...
        curl_setopt($ch, CURLOPT_VERBOSE, $this->configuration->curlOptVerbose);
    ...
    {
        $header = "Bearer ".$this->configuration->accessToken; // Bearer 다음에 공백 추가
    ...
    {
        $url = $this->configuration->host."listCategory.json";
